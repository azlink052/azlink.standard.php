...
if (class_exists('Transform')) return;

use azlink\workspace\config as config;
use azlink\workspace\classes as classes;
...
class Transform {
	...
	static function convert($val, $change) {
		...
		if (is_array($val)) {
			...
			$val = mb_convert_kana($val, $change, config\PROG_ENCTYPE);
		}
	...
	static function serialize($val) {
		...
		if (is_array($val)) {
			...
			if (!classes\common\ArrayHelper::checkArrayNull($val)) {
				$val = serialize($val);
			} else {
				$val = '';
			}
	...
	static function decodeCSV($val) {
		...
		return mb_convert_encoding($val, config\CSV_ENCTYPE, config\DB_ENCTYPE);
