...
class RequiredCheck {
	 ...
	 * @param チェックする文字列
	 */
	static function checkAlphaNumUBarComma($val) {
		if (!$val) return 0;
	 ...
	 * @param チェックする文字列
	 */
	static function checkAlphaNumUBarCommaHyphen($val) {
		if (!$val) return 0;
	...
	static function checkZenkakuOnly($val, $encoding = NULL) {
		...
		for ($i = 0; $i < $len; $i++) {
			...
			if (RequiredCheck::checkHankakuOnly($char, TRUE, TRUE, $encoding)) return 1;
		}
		return 0;
	}
	/**
	...
	static function checkDate($month, $day, $year) {
		if (!$month || !$day || !$year ||
		RequiredCheck::checkNumeric($month) ||
		RequiredCheck::checkNumeric($day) ||
		RequiredCheck::checkNumeric($year)) return 0;
	...
	static function checkURL($val) {
		...
		$result = (!preg_match('/^(http|HTTP|ftp)(s|S)?:\/\/+[A-Za-z0-9]+\.[A-Za-z0-9]/', $url)) ? 1 : 0;
		return $result;
	}
	/**
	 * 4バイト文字が混ざっていないかチェック
